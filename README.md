...
# Test Thanh Tra
## Setup
- Run project on localhost with port 3000.
- Into the directory `./src/assets/styles/layout/_global.scss` hidden properties `overflow-x: hidden`. Because the overflow-x: hidden attribute will hide the scroll, webdriverio will not action scroll to element.

- PhantomJS can't test promise of ES6 should add scripts `<script src="https://cdnjs.cloudflare.com/ajax/libs/core-js/2.4.1/core.min.js"></script>`
into `<head>` in file `index.html` of project.
## Run test
- Test with Chrome : `npm run test:chrome`
- Test with Phantomjs : `npm run test:phantomjs`
## Test single feature
- You can setup multiple configs run test single feature: Go to the file `package.json` configs directory path file gherkin in `test:single-chrome` or `test:single-phantomjs`.
...
```sh
"test:single":"wdio --spec ./src/features/path file gherkin..."
```
Phantomjs

```sh
"test:single":"wdio wdio.BUILD.conf.js --spec ./src/features/path file gherkin..."
```
Run test :
...
- You can setup multiple configs in file `package.json` :
...
```sh
"test:tags":"wdio --tags=@Tag,@AnotherTag..."
```
Phantomjs

```sh
"test:tags":"wdio wdio.BUILD.conf.js --tags=@Tag,@AnotherTag..."
```
...
Read more : [here](http://webdriver.io/guide/reporters/spec.html)
